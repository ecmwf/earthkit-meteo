...
@pytest.mark.parametrize(
    ...
    [(datetime.datetime(2024, 4, 22), datetime.datetime(2024, 4, 23), 40.0, 18.0, 0.3110738757)],
)
def test_cos_solar_zenith_angle_integrated(begin_date, end_date, lat, lon, v_ref, array_backend):
    lat, lon, v_ref = array_backend.asarray(lat, lon, v_ref)
    v = solar.cos_solar_zenith_angle_integrated(begin_date, end_date, lat, lon)
    v_ref = array_backend.asarray(v_ref, dtype=v.dtype)
    assert array_backend.allclose(v, v_ref)
