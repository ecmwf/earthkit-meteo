...
def save_cape_cin_reference(expected_data):
    """Regenerate the CSV reference file from dicts of 1-D arrays."""
    np.savetxt(
...
class CapeCinData:
    ...
    p_sfc, t_sfc, zh_sfc, r_sfc : dict[str, np.ndarray]
    ...
    p_sfc_stacked, t_sfc_stacked, zh_sfc_stacked, r_sfc_stacked : np.ndarray
    ...
    def __init__(self):
        ...
        self.r = {n: raw[f"{n}_r"][:-1] for n in CASE_NAMES}
        ...
        self.r_sfc = {n: raw[f"{n}_r"][-1] for n in CASE_NAMES}
        ...
        self.r_sfc_stacked = np.array([self.r_sfc[n] for n in CASE_NAMES])
...
def test_cape_cin(case_name, parcel_type):
    ...
    r_sfc = np.atleast_1d(data.r_sfc[case_name])

    cape, cin = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, parcel_type)
...
def test_cape_cin_arbitrary_nd_shape():
    ...
    r = np.broadcast_to(np.array([0.03, 0.02, 0.01])[:, None, None, None], shape)
    ...
    r_sfc = np.full(horizontal_shape, 0.008)
...
def test_cape_cin_lat_lon():
    ...
    r_sfc = data.r_sfc_stacked.reshape(ny, nx)
...
def test_cape_cin_options_forwarded():
    ...
    r_sfc = np.atleast_1d(data.r_sfc["unstable"])

    # Default layer_depth (5000 Pa) vs a wider mixed layer (15000 Pa) must differ.
    cape_default, _ = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "mixed")
    cape_wide, _ = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "mixed", layer_depth=15000)
...
def test_cape_cin_invalid_parcel_type():
    ...
    with pytest.raises(ValueError, match="parcel_type"):
        thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "unknown_parcel")
...
def test_cape_cin_all_nan_profile():
    ...
    r_sfc = np.array([np.nan, data.r_sfc["stable"]])
...
def test_cape_cin_no_lfc():
    ...
    r_sfc = np.atleast_1d(data.r_sfc["stable"])
...
def test_cape_cin_unsorted_pressure():
    ...
    for parcel_type in PARCEL_TYPES:
        cape_sorted, cin_sorted = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, parcel_type)
        cape_flip, cin_flip = thermo.cape_cin(p_flip, zh_flip, t_flip, r_flip, p_sfc, zh_sfc, t_sfc, r_sfc, parcel_type)
...
def test_cape_cin_unsorted_pressure_stacked():
    ...
    r_sfc_1d = np.atleast_1d(data.r_sfc["unstable"])
    ...
    r_sfc2 = np.concatenate([r_sfc_1d, r_sfc_1d])

    for parcel_type in PARCEL_TYPES:
        cape, cin = thermo.cape_cin(p2, zh2, t2, r2, p_sfc2, zh_sfc2, t_sfc2, r_sfc2, parcel_type)
        np.testing.assert_allclose(
            cape[0],
            cape[1],
            atol=1,
...
def _unstable_1col():
    """Return (p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc) for the 'unstable' case.
    ...
    return (
        ...
        np.atleast_1d(data.r_sfc["unstable"]),
    )


def test_extra_outputs_none_returns_two_tuple():
    """When extra_outputs is not set the return value must be a 2-tuple."""
    p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc = _unstable_1col()
    result = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "surface")
...
def test_extra_outputs_empty_list_returns_two_tuple():
    ...
    p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc = _unstable_1col()
    result = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "surface", extra_outputs=[])
    assert len(result) == 2
...
def test_extra_outputs_parcel_path_shape():
    ...
    cape, cin, extras = thermo.cape_cin(
        p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "surface", extra_outputs=["parcel_path"]
    )
...
def test_extra_outputs_standalone_key_levels():
    ...
    p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc = _unstable_1col()
    cape, cin, extras = thermo.cape_cin(
        p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "surface", extra_outputs=["lcl", "lfc", "el", "parcel"]
    )
...
def test_extra_outputs_key_levels_consistent_with_parcel_path():
    ...
    p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc = _unstable_1col()
    cape, cin, extras = thermo.cape_cin(
        p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "surface", extra_outputs=["lcl", "lfc", "el", "parcel", "parcel_path"]
    )
    path = extras["parcel_path"]
...
def test_extra_outputs_cape_cin_values_unchanged():
    ...
    p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc = _unstable_1col()
    cape_base, cin_base = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "surface")
...
def test_extra_outputs_invalid_key_raises():
    ...
    with pytest.raises(ValueError, match="extra_outputs"):
        thermo.cape_cin(
            p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "surface", extra_outputs=["parcel_path", "not_a_real_key"]
        )
...
def test_extra_outputs_parcel_path_pressure_sorted():
    ...
    _, _, extras = thermo.cape_cin(
        p_flip, zh_flip, t_flip, r_flip, p_sfc, zh_sfc, t_sfc, r_sfc, "surface", extra_outputs=["parcel_path"]
    )
...
def test_extra_outputs_vertical_axis_minus_1():
    ...
    cape, cin, extras = thermo.cape_cin(
        p_T, zh_T, t_T, r_T, p_sfc, zh_sfc, t_sfc, r_sfc, "surface", vertical_axis=-1, extra_outputs=["parcel_path"]
    )
...
def test_extra_outputs_vertical_axis_arbitrary():
    ...
    cape, cin, extras = thermo.cape_cin(
        p_v2, zh_v2, t_v2, r_v2, p_sfc, zh_sfc, t_sfc, r_sfc, "surface", vertical_axis=2, extra_outputs=["parcel_path"]
    )
...
def test_cape_cin_subground_levels_masked():
    ...
    cape_ref, cin_ref = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "surface")
    ...
    r_extra = np.array([[0.1], [0.2]])  # bogus

    p_aug = np.concatenate([p, p_extra], axis=0)
    zh_aug = np.concatenate([zh, zh_extra], axis=0)
    t_aug = np.concatenate([t, t_extra], axis=0)
    r_aug = np.concatenate([r, r_extra], axis=0)

    for parcel_type in PARCEL_TYPES:
        cape, cin = thermo.cape_cin(p_aug, zh_aug, t_aug, r_aug, p_sfc, zh_sfc, t_sfc, r_sfc, parcel_type)
        cape_ref, cin_ref = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, parcel_type)
        np.testing.assert_allclose(
            cape,
            cape_ref,
            atol=1,
...
def test_cape_cin_stacked_different_surfaces():
    ...
    r = np.column_stack([data.r["elevated_instability"], data.r["stable"]])
    ...
    r_sfc = np.array([data.r_sfc["elevated_instability"], data.r_sfc["stable"]])
...
def test_cape_cin_mixed_uses_surface_pressure():
    ...
    r = data.r["elevated_instability"][:, None]
    ...
    r_sfc = np.atleast_1d(data.r_sfc["elevated_instability"])

    _, _, extras = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "mixed", extra_outputs=["parcel"])
...
def test_cape_cin_surface_parcel_uses_surface_inputs():
    ...
    _, _, extras = thermo.cape_cin(p, zh, t, r, p_sfc, zh_sfc, t_sfc, r_sfc, "surface", extra_outputs=["parcel"])
    np.testing.assert_allclose(extras["parcel"].p, p_sfc)
    np.testing.assert_allclose(extras["parcel"].t, t_sfc)
    np.testing.assert_allclose(extras["parcel"].r, r_sfc)
